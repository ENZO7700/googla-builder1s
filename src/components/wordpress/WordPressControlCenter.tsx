...
const CAPABILITY_GROUPS: CapabilityGroup[] = [
  ...
  {
    title: 'Rizikové a zamknuté',
    description: 'Tieto akcie potrebujú samostatný confirm krok.',
    icon: <LockKeyhole size={16} />,
    items: [
      { name: 'Delete post', detail: 'Metóda existuje, ale nesmie ísť bez jasného potvrdenia.', status: 'locked' },
      { name: 'Update settings', detail: 'Zmena nastavení webu ostáva uzamknutá.', status: 'locked' },
      { name: 'Plugin activate/deactivate', detail: 'Zmena pluginov potrebuje potvrdenie a audit.', status: 'locked' },
      { name: 'WP-CLI maintenance/cache', detail: 'Spúšťať iba cez whitelist a explicitný confirm.', status: 'locked' },
    ],
  },
];

const SUPABASE_PROJECT_ID = 'lmuervovjnpadapfwwmh';
      ...
      <div className="grid gap-6 p-5 xl:grid-cols-[1.15fr_0.85fr]">
        ...
        <section className="rounded-2xl border border-border bg-background/70 p-4">
          ...
          <div className="mt-4 rounded-xl border border-warning/20 bg-warning/10 p-3 text-xs leading-5 text-muted-foreground">
            ...
            Po úspešnom teste vie Supabase uložiť pripojenie cez `wordpress-connection`. Write akcie ostávajú uzamknuté, kým nedostanú samostatný confirm flow.
          </div>
...
function CapabilityBadge({ status }: { status: CapabilityItem['status'] }) {
  ...
  return <StatusBadge tone="muted" label="locked" />;
}
